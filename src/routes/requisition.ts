...
router.get('/orders/waiting', async (req, res, next) => {
  ...
  try {
    let rs: any = await orderModel.getListWaiting(db, null, warehouseId, limit, offset, query);
    let total: any = await orderModel.totalListWaiting(db, null, warehouseId, query);
...
router.get('/orders/waiting-approve', async (req, res, next) => {
  ...
  try {
    let rs: any = await orderModel.getListWaitingApprove(db, null, warehouseId, limit, offset, query);
    let total: any = await orderModel.totalListWaitingApprove(db, null, warehouseId, query);
    res.send({ ok: true, rows: rs[0], total: total[0] });
  } catch (error) {
    res.send({ ok: false, error: error.message });
  } finally {
...
router.get('/orders/unpaid', async (req, res, next) => {
  ...
  try {
    let rs: any = await orderModel.getUnPaidOrders(db, null, warehouseId, limit, offset, query);
    let total: any = await orderModel.totalUnPaidOrders(db, null, warehouseId, query);
    res.send({ ok: true, rows: rs[0], total: total[0] });
  } catch (error) {
    res.send({ ok: false, error: error.message });
  } finally {
