  ...
  try {
    ...
    if (!receiveIds.length) {
      ...
      for (const v of _rproducts) {
        ...
        let id = uuid();
        await receiveModel.updateReceiveOtherDetailSummary(db, v.receive_detail_id, { 'wm_product_id': id });
        let obj: any = {
          wm_product_id: id,
          warehouse_id: v.warehouse_id,
          product_id: v.product_id,
