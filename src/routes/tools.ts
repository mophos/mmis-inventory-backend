...
router.put('/stockcard/borrow', async (req, res, next) => {
  ...
  let peopleUserId = req.decoded.people_user_id;
  try {
    await toolModel.updateBorrows(db, borrowId, summary);
    for (const v of generics) {
      await toolModel.updateBorrowGenerics(db, borrowId, v.generic_id, v.unit_generic_id, v.borrow_qty);
      for (const i of v.products) {
        ...
        if (stockCardIdOut.length && stockCardIdIn.length) {
          ...
          if (qtyNew > qtyOld) {
            qty = qtyNew - qtyOld;
            await toolModel.decreaseQty(db, i.product_id, i.lot_no, i.lot_time, summary.srcWarehouseId, qty) // ลดลง
          } else if (qtyNew < qtyOld) {
            qty = qtyOld - qtyNew;
            await toolModel.increasingQty(db, i.product_id, i.lot_no, i.lot_time, summary.srcWarehouseId, qty) // เพิ่มขึ้น
          }

          if (qtyNew > qtyOld) {
            qty = qtyNew - qtyOld;
            await toolModel.increasingQty(db, i.product_id, i.lot_no, i.lot_time, summary.dstWarehouseId, qty) // เพิ่มขึ้น
          } else if (qtyNew < qtyOld) {
            qty = qtyOld - qtyNew;
            await toolModel.decreaseQty(db, i.product_id, i.lot_no, i.lot_time, summary.dstWarehouseId, qty) // ลดลง
          }
          // #####################################
          await toolModel.updateBorrowProducts(db, v.borrow_generic_id, i.wm_product_id, qtyNew);
          ...
          lists = await toolModel.getStockcardList(db, summary.srcWarehouseId, v.generic_id); // รายการทั้งหทก
          productId = await toolModel.getStockcardProduct(db, summary.dstWarehouseId, v.generic_id); //product id
          for (const pd of productId) {
            const obj: any = {
              generic_id: v.generic_id,
              product_id: pd.product_id,
