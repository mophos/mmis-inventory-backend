...
WHERE
  ...
  getPurchaseProductList(knex: Knex, purchaseOrderId: any) {
    let sql = `
    select (SELECT location_id FROM wm_receive_detail WHERE product_id = pi.product_id ORDER BY receive_detail_id DESC LIMIT 1) AS location_id,pi.product_id, p.product_name, pi.unit_generic_id,
