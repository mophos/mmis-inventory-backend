...
export class ReceiveModel {
  ...
  getReceiveWaitingOtherTotalSearch(knex: Knex, query: string) {
    let _query = `%${query}%`;

    return knex('wm_receive_other as r')
      .leftJoin('wm_receive_other_detail as rd', 'rd.receive_other_id', 'r.receive_other_id')
      .where('r.receive_code', 'like', _query)
      // .orWhere('rd.', 'like', _query)
      .count('* as total');
  }
