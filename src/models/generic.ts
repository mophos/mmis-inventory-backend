...
export class GenericModel {
  ...
  warehouseSearchAutocompleteLimit(knex: Knex, warehouseId: any, q: any) {
    ...
    return knex.raw(sql, { q, q_, _q_, warehouseId });
  }
  ...
  warehouseSearchAutocompleteLimitStaff(knex: Knex, warehouseId: any, q: any, srcWarehouseId: any) {
    ...
    return knex.raw(sql, { q, q_, _q_, warehouseId, srcWarehouseId });
  }
  ...
  updateGeneric(knex: Knex, warehouseId: any) {
    let sql = `
      update mm_generics t,
      (
        select g.generic_id, gp.min_qty, gp.max_qty
        from mm_generics g
        join mm_generic_planning gp on gp.generic_id = g.generic_id and gp.warehouse_id = ?
      ) s
