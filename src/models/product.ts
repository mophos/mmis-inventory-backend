...
export class ProductModel {
  ...
  adminSearchAllProductsLabelerLimit(knex: Knex, query: any, labelerId: any, genericTypeId) {
    let q_ = `${query}%`;
    let _q_ = `%${query}%`;
    let sql = `
  ...
  adminSearchAllProductsLabelerAll(knex: Knex, query: any, labelerId: any, genericTypeId: any) {
    let q_ = `${query}%`;
    let _q_ = `%${query}%`;
    let sql = `
    select DISTINCT * from (
  ...
  adminSearchAllProductsLimit(knex: Knex, query: any, genericTypeId: any) {
    let q_ = `${query}%`;
    let _q_ = `%${query}%`;
    let sql = `
    select DISTINCT * from (
  ...
  adminSearchAllProductsAll(knex: Knex, query: any, genericTypeId: any) {
    let q_ = `${query}%`;
    let _q_ = `%${query}%`;
    let sql = `
    select DISTINCT * from (
...
group by mpp.product_id
  ...
  adminSearchProducts(knex: Knex, query: any, productGroups: any = [], genericType: any, warehouseId: any, limit: number, offset: number, sort: any = {}) {
    ...
    if (genericType) {
      ...
      return knex.raw(sql, [_query, _query, query, query, _query, limit, offset]);
    } else {
      let sql = `
    select p.wm_product_id, p.product_id, sum(p.qty) as qty, sum(p.qty * p.cost) as total_cost,
        mp.product_name, mp.working_code, g.generic_name, g.working_code as generic_working_code, mp.primary_unit_id, u.unit_name as primary_unit_name,
  ...
  adminSearchProductsTotal(knex: Knex, query: any, productGroups: any = [], genericType: any, warehouseId: any) {
    ...
    if (genericType) {
      ...
      sql += `group by p.product_id
      order by mp.product_name`
      return knex.raw(sql, [_query, _query, query, query, _query]);
    } else {
      let sql = `
      select p.wm_product_id, p.product_id, sum(p.qty) as qty, sum(p.qty * p.cost) as total_cost,
        mp.product_name, g.generic_name, mp.primary_unit_id, u.unit_name as primary_unit_name
  ...
  getAllProduct(db: Knex, genericType: any = {}) {
    ...
    sql.orderBy('mp.product_name', 'DESC');
